...
RSpec.describe 'create customer endpoint', type: :request do
    describe 'happy path' do
        it 'can create a customer' do
            ...
            expect(confirmation[:data][:attributes][:first_name]).to eq("dane")
            expect(confirmation[:data][:attributes][:last_name]).to eq("brophy")
            expect(confirmation[:data][:attributes][:email]).to eq("dbrophy@cats.biz")
            expect(confirmation[:data][:attributes][:address]).to eq("101 Generic Drive, Ware, CO")
        end
    end
